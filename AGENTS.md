...
## Cloudflare Pages Deployment (Standard Spec)

This boilerplate is configured for Cloudflare Pages deployment via either the CLI or Cloudflare dashboard GitHub integration.

### Required Configuration

- Keep `wrangler.jsonc` at the repository root.
- Ensure `pages_build_output_dir` is set to `./dist` (Astro static output).
- Keep `env.preview` defined for non-production/preview deployments.
- Keep an `env.production` custom domain example commented out by default, then uncomment when production domains are ready.

### Deployment Paths

1. **CLI deploys**
   - `pnpm deploy` → production Pages deployment
   - `pnpm deploy:preview` → preview deployment (branch-based)
2. **Cloudflare dashboard (GitHub App integration)**
   - Build command: `pnpm build`
   - Build output directory: `dist`
   - `wrangler.jsonc` is read automatically by Pages builds.

### URL Expectations

- Production deploys are available at `<project>.pages.dev`.
- Non-production branches and PRs get preview URLs at `<branch>.<project>.pages.dev` (and unique hash URLs per deployment).

### Types and Local Tooling

- Use `pnpm cf-typegen` after changing `wrangler.jsonc` to regenerate runtime types.
- Do not commit local/state artifacts generated by Wrangler (see Ignored Paths section).
...
| `deploy`          | `pnpm build && wrangler pages deploy`                  | Build and deploy to Cloudflare Pages                    |
| `deploy:preview`  | `pnpm build && wrangler pages deploy --branch=preview` | Build and deploy preview to Cloudflare Pages            |
| `cf-typegen`      | `wrangler types`                                       | Generate Cloudflare runtime types from `wrangler.jsonc` |
...
- `.wrangler/` + `.dev.vars` — Wrangler local state/secrets
