...
| Deployment         | Cloudflare Workers + Wrangler | ^4.x               |
...
## Cloudflare Workers Deployment (Standard Spec)

This boilerplate is configured for Cloudflare Workers deployment using Wrangler.

### Required Configuration

- Keep `wrangler.jsonc` at the repository root.
- Ensure `assets.directory` is set to `./dist` (Astro static output).
- Keep `env.preview` defined for non-production/preview deployments.
- Keep an `env.production` routes example commented out by default, then uncomment when production routes/domains are ready.

### Deployment Paths

1. **CLI deploys**
   - `pnpm deploy` → production Workers deployment
   - `pnpm deploy:preview` → preview Workers deployment (`--env preview`)

### URL Expectations

- Production deploys are available at `<worker-name>.<account-subdomain>.workers.dev` unless routes override it.
- Preview deploys use the preview environment worker name (typically `<worker-name>-preview.<account-subdomain>.workers.dev`).
...
| `deploy`          | `pnpm build && wrangler deploy`               | Build and deploy to Cloudflare Workers                  |
| `deploy:preview`  | `pnpm build && wrangler deploy --env preview` | Build and deploy preview env to Cloudflare Workers      |
